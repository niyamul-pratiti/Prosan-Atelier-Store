<?php $__env->startSection('title', $product->meta_title ?: $product->name.' - Prosan Atelier'); ?>
<?php $__env->startSection('meta_description', $product->meta_description ?: Str::limit(strip_tags($product->short_description ?: $product->description), 150)); ?>
...
<section class="py-5" style="background-image:url('<?php echo e(asset('foodmart/images/background-pattern.jpg')); ?>');background-size:cover;">
    <div class="container-fluid">
        <nav class="breadcrumb"><a href="<?php echo e(route('home')); ?>">Home</a><span>/</span><a href="<?php echo e(route('shop.index')); ?>">Shop</a><span>/</span><span><?php echo e($product->name); ?></span></nav>
    </div>
</section>
...
<section class="py-5 product-detail-section">
    <div class="container-fluid">
        <div class="row g-5 align-items-start">
            <div class="col-lg-6">
                <div class="single-product-image rounded-5 bg-light p-5 text-center"><img src="<?php echo e($product->image_url); ?>" class="img-fluid" alt="<?php echo e($product->name); ?>"></div>
                <?php if($galleryImages->count()): ?>
                    <div class="d-flex gap-3 mt-3 flex-wrap">
                        <?php $__currentLoopData = $galleryImages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $image): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <img src="<?php echo e($image->url); ?>" class="product-thumb-small" alt="<?php echo e($image->alt_text ?: $product->name); ?>">
            ...
            <div class="col-lg-6">
                <div class="product-summary foodmart-product-summary">
                    <?php if($displayCategory): ?>
                        <a class="product-category-eyebrow text-primary fw-bold mb-2" href="<?php echo e(route('category.show', $displayCategory->slug)); ?>"><?php echo e($displayCategory->name); ?></a>
                    <?php else: ?>
                        <a class="product-category-eyebrow text-primary fw-bold mb-2" href="<?php echo e(route('shop.index')); ?>">Shop</a>
                    <?php endif; ?>
                    <h1 class="display-6 fw-bold"><?php echo e($product->name); ?></h1>
                    <div class="rating mb-3"><svg width="24" height="24" class="text-primary"><use xlink:href="#star-solid"></use></svg> 4.5 <span class="text-muted ms-2">Brand: <?php echo e($product->brand->name ?? 'No Brand'); ?></span></div>
                    ...
                    <form class="product-form card border-0 shadow-sm rounded-4 p-4 js-ajax-add-to-cart" method="POST" action="<?php echo e(route('cart.add')); ?>" id="productAddToCartForm">
                        <?php echo csrf_field(); ?>
                            ...
                            <div class="mb-3 variation-picker-box">
                                ...
                                <select class="form-select form-select-lg" name="variation_id" id="variationSelect" required>
                                    ...
                                    <?php $__currentLoopData = $product->variations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $variation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($variation->id); ?>" <?php if($variation->stock_quantity <= 0): echo 'disabled'; endif; ?>>
                                            <?php echo e($variation->name); ?> — ৳<?php echo e(number_format($variation->effective_price, 0)); ?><?php echo e($variation->stock_quantity <= 0 ? ' (Out of stock)' : ' (Stock: '.$variation->stock_quantity.')'); ?>
                    ...
                    <form method="POST" action="<?php echo e(route('wishlist.toggle', $product)); ?>" class="mt-3">
                        <?php echo csrf_field(); ?>
                    ...
                    <div class="row g-3 mt-4 product-meta-grid">
                        <div class="col-sm-6"><div class="info-pill"><strong>SKU:</strong> <span id="productSkuDisplay"><?php echo e($product->sku ?: 'N/A'); ?></span></div></div>
                        <div class="col-sm-6"><div class="info-pill"><strong>Stock:</strong> <span id="productStockDisplay"><?php echo e($product->is_variable ? 'Select option' : $product->stock_quantity); ?></span></div></div>
                        <div class="col-sm-6"><div class="info-pill"><strong>Category:</strong> <?php if($displayCategory): ?><a href="<?php echo e(route('category.show', $displayCategory->slug)); ?>"><?php echo e($displayCategory->name); ?></a><?php else: ?> N/A <?php endif; ?></div></div>
                        <div class="col-sm-6"><div class="info-pill"><strong>Weight:</strong> <span id="productWeightDisplay"><?php echo e($product->weight && ! $product->is_variable ? $product->weight . ' ' . $product->unit : 'N/A'); ?></span></div></div>
                    </div>
                </div>
            </div>
        </div>
...
<?php if($product->description || $product->ingredients || $product->usage_instruction): ?>
    ...
    <div class="container-fluid">
        <div class="card border-0 rounded-5 p-4 p-md-5">
            ...
            <div class="tab-content pt-4">
                <div class="tab-pane fade show active" id="desc"><?php echo nl2br(e($product->description)); ?></div>
                <div class="tab-pane fade" id="ingredients"><?php echo nl2br(e($product->ingredients ?: 'Not specified.')); ?></div>
                <div class="tab-pane fade" id="usage"><?php echo nl2br(e($product->usage_instruction ?: 'Not specified.')); ?></div>
            </div>
        </div>
    </div>
</section>
...
<section class="py-5 overflow-hidden related-products-section">
    <div class="container-fluid">
        <div class="section-header prosan-section-header d-flex justify-content-between align-items-center"><h2 class="section-title">Related Products</h2><a href="<?php echo e(route('shop.index')); ?>" class="btn-link text-decoration-none">View Shop →</a></div>
        <div class="products-carousel swiper"><div class="swiper-wrapper"><?php $__currentLoopData = $relatedProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $related): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php echo $__env->make('partials.product-card', ['product' => $related, 'carousel' => true], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></div></div>
    </div>
</section>
...
<script>
(function () {
    const variations = <?php echo json_encode($variationPayload, 15, 512) ?>;
    ...
    function renderVariation() {
        ...
        if (!selected) {
            priceDisplay.innerHTML = <?php echo json_encode($product->price_label, 15, 512) ?>;
            skuDisplay.textContent = <?php echo json_encode($product->sku ?: 'N/A', 15, 512) ?>;
        ...
        skuDisplay.textContent = selected.sku || <?php echo json_encode($product->sku ?: 'N/A', 15, 512) ?>;
...
<?php echo $__env->make('layouts.store', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/niyamulp/prosanatelier_app/resources/views/frontend/product.blade.php ENDPATH**/ ?>