...
<html lang="en">
<head>
    <title>@yield('title', $siteSettings['site_name'] ?? 'Prosan Atelier')</title>
    <meta charset="utf-8">
    ...
    <meta name="description" content="@yield('meta_description', $siteSettings['meta_description'] ?? 'Prosan Atelier - Korean, Thai, Chinese food items, cooking essentials and cosmetics.')">
    ...
    <link rel="stylesheet" type="text/css" href="{{ asset('foodmart/css/prosan-foodmart.css') }}?v=20260729-1">
...
<div class="offcanvas offcanvas-end" data-bs-scroll="true" tabindex="-1" id="offcanvasCart" aria-labelledby="My Cart">
    ...
    <div class="offcanvas-body" id="prosan-cart-panel-body">
        @include('partials.cart-offcanvas-body', ['cart' => $cart, 'cartCount' => $cartCount, 'cartTotal' => $cartTotal])
    </div>
</div>
...
<div class="offcanvas offcanvas-end" data-bs-scroll="true" tabindex="-1" id="offcanvasSearch" aria-labelledby="Search">
    ...
    <div class="offcanvas-body"><div class="order-md-last">
        ...
        <form role="search" action="{{ route('shop.index') }}" method="get" class="d-flex mt-3 gap-0">
...
<header class="prosan-site-header">
    <div class="container-fluid">
        <div class="row py-3 border-bottom align-items-center prosan-header-main">
            <div class="col-6 col-lg-3 prosan-logo-col">
                <div class="main-logo prosan-main-logo"><a href="{{ route('home') }}"><img src="{{ $storeLogoUrl }}" alt="{{ $storeSiteName }}" class="img-fluid"></a></div>
            </div>
            <div class="col-lg-5 d-none d-lg-block">
                <div class="search-bar row bg-light p-2 my-2 rounded-4">
                    <div class="col-md-4 d-none d-md-block">
                        <select class="form-select border-0 bg-transparent" name="category" form="search-form">
                            ...
                            @foreach($menuCategories as $headerCategory)
                                <option value="{{ $headerCategory->slug }}" @selected(request('category') === $headerCategory->slug)>{{ $headerCategory->name }}</option>
                    ...
                    <div class="col-11 col-md-7">
                        <form id="search-form" class="text-center" action="{{ route('shop.index') }}" method="get">
                            <input type="text" name="q" value="{{ request('q') }}" class="form-control border-0 bg-transparent" placeholder="Search Korean food, cosmetics, cooking essentials" />
                        </form>
                    </div>
            ...
            <div class="col-6 col-lg-4 d-flex justify-content-end align-items-center prosan-header-actions">
                <div class="support-box text-end d-none d-xl-block"><span class="fs-6 text-muted">For Support?</span><h5 class="mb-0">{{ $supportPhone }}</h5></div>
                <ul class="header-icon-list d-flex justify-content-end align-items-center list-unstyled m-0">
                    <li><a href="{{ session('customer_id') ? route('customer.dashboard') : route('customer.login') }}" class="prosan-circle-icon" title="My Account"><svg width="24" height="24"><use xlink:href="#user"></use></svg></a></li>
                    <li><a href="{{ route('order.tracking') }}" class="prosan-circle-icon" title="Track Order"><svg width="24" height="24"><use xlink:href="#link"></use></svg></a></li>
                    <li class="d-lg-none"><a href="#" class="prosan-circle-icon prosan-cart-trigger" data-bs-toggle="offcanvas" data-bs-target="#offcanvasCart" title="Cart"><svg width="24" height="24"><use xlink:href="#cart"></use></svg><span class="cart-count-badge" data-cart-count>{{ $cartCount }}</span></a></li>
                ...
                <div class="cart text-end d-none d-lg-block dropdown ms-4">
                    <button class="border-0 bg-transparent d-flex flex-column gap-2 lh-1" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasCart">
                        <span class="fs-6 text-muted dropdown-toggle">Your Cart <span class="cart-count-inline" data-cart-count>{{ $cartCount }}</span></span><span class="cart-total fs-5 fw-bold" data-cart-total>৳{{ number_format($cartTotal, 0) }}</span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="container-fluid nav-wrap-prosan d-none d-lg-block">
        <div class="prosan-desktop-nav py-3">
            <div class="department-dropdown">
                <a class="filter-categories border-0 mb-0 d-inline-flex align-items-center" href="{{ route('shop.index') }}">Shop by Departments <svg width="18" height="18" class="ms-2"><use xlink:href="#arrow-right"></use></svg></a>
                <div class="department-mega shadow-lg">
                    <div class="row g-4">
                        <div class="col-lg-8">
                            <h5>Browse Categories</h5>
                            <div class="mega-grid-foodmart">
                                @foreach($menuCategories as $megaCategory)
                                    <div class="mega-item-foodmart">
                                        <a href="{{ route('category.show', $megaCategory->slug) }}" class="mega-title"><img src="{{ $megaCategory->image_url }}" alt="{{ $megaCategory->name }}"> <span>{{ $megaCategory->name }}</span></a>
                                        @foreach(($megaCategory->children ?? collect())->take(5) as $child)
                        ...
                        <div class="col-lg-4">
                            <div class="mega-offer-card">
                                <span>Fresh Picks</span><h4>Asian food + cosmetics in one place.</h4><a href="{{ route('shop.index') }}">Shop collection →</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <ul class="prosan-desktop-menu navbar-nav menu-list list-unstyled mb-0">
                ...
                <li class="nav-item"><a href="{{ route('shop.index', ['category' => 'ramen']) }}" class="nav-link">Ramen</a></li>
                <li class="nav-item"><a href="{{ route('shop.index', ['category' => 'cooking-essentials']) }}" class="nav-link">Cooking Essentials</a></li>
                <li class="nav-item"><a href="{{ route('shop.index', ['category' => 'cosmetics']) }}" class="nav-link">Cosmetics</a></li>
                <li class="nav-item"><a href="{{ route('order.tracking') }}" class="nav-link">Track Order</a></li>
                <li class="nav-item"><a href="{{ session('customer_id') ? route('customer.dashboard') : route('customer.login') }}" class="nav-link">My Account</a></li>
            </ul>
        </div>
    </div>
    ...
    <div class="offcanvas offcanvas-start prosan-mobile-offcanvas" tabindex="-1" id="offcanvasNavbar" aria-labelledby="mobileMenuLabel">
        <div class="offcanvas-header mobile-offcanvas-head">
            <a href="{{ route('home') }}" class="mobile-offcanvas-logo"><img src="{{ $storeLogoUrl }}" alt="{{ $storeSiteName }}"></a>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body mobile-offcanvas-body">
            <a class="mobile-department-link" href="{{ route('shop.index') }}">Shop by Departments <span>→</span></a>
            <div class="mobile-category-list">
                @foreach($menuCategories as $mobileCategory)
                    <a href="{{ route('category.show', $mobileCategory->slug) }}"><img src="{{ $mobileCategory->image_url }}" alt="{{ $mobileCategory->name }}"><span>{{ $mobileCategory->name }}</span></a>
                @endforeach
            </div>
            <ul class="mobile-menu-list list-unstyled mb-0">
                <li><a href="{{ route('home') }}">Home</a></li>
                <li><a href="{{ route('shop.index') }}">Shop</a></li>
                <li><a href="{{ route('shop.index', ['category' => 'ramen']) }}">Ramen</a></li>
                <li><a href="{{ route('shop.index', ['category' => 'cooking-essentials']) }}">Cooking Essentials</a></li>
                <li><a href="{{ route('shop.index', ['category' => 'cosmetics']) }}">Cosmetics</a></li>
                <li><a href="{{ route('order.tracking') }}">Track Order</a></li>
                <li><a href="{{ session('customer_id') ? route('customer.dashboard') : route('customer.login') }}">My Account</a></li>
                @if(session('customer_id'))<li><a href="{{ route('customer.wishlist') }}">Wishlist</a></li>@endif
            </ul>
        </div>
    </div>
</header>
...
<main>
    <div class="container-fluid pt-3">@include('partials.flash')</div>
    @yield('content')
</main>
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-3 col-md-6 col-sm-6">
                <div class="footer-menu"><img src="{{ $storeLogoUrl }}" alt="{{ $storeSiteName }}" class="footer-logo-img"><p>{{ $footerText }}</p><p><strong>Call/WhatsApp:</strong> {{ $supportPhone }}</p>@if($footerSocials->isNotEmpty())<div class="footer-socials" aria-label="Social links">@foreach($footerSocials as $social)<a href="{{ $social['url'] }}" class="footer-social-icon footer-social-{{ $social['key'] }}" target="_blank" rel="noopener noreferrer" aria-label="{{ $social['label'] }}"><svg><use xlink:href="#{{ $social['icon'] }}"></use></svg><span class="visually-hidden">{{ $social['label'] }}</span></a>@endforeach</div>@endif</div>
            </div>
            <div class="col-md-2 col-sm-6"><div class="footer-menu"><h5 class="widget-title">Shop</h5><ul class="menu-list list-unstyled"><li><a href="{{ route('shop.index') }}" class="nav-link">All Products</a></li><li><a href="{{ route('shop.index', ['category'=>'ramen']) }}" class="nav-link">Ramen</a></li><li><a href="{{ route('shop.index', ['category'=>'seaweed']) }}" class="nav-link">Seaweed</a></li><li><a href="{{ route('shop.index', ['category'=>'skin-care']) }}" class="nav-link">Skin Care</a></li></ul></div></div>
            <div class="col-md-2 col-sm-6"><div class="footer-menu"><h5 class="widget-title">Brands</h5><ul class="menu-list list-unstyled">@foreach($menuBrands->take(5) as $brand)<li><a href="{{ route('brand.show', $brand->slug) }}" class="nav-link">{{ $brand->name }}</a></li>@endforeach</ul></div></div>
            <div class="col-md-2 col-sm-6"><div class="footer-menu"><h5 class="widget-title">Customer Service</h5><ul class="menu-list list-unstyled"><li><a href="{{ route('cart.index') }}" class="nav-link">Cart</a></li><li><a href="{{ route('checkout.index') }}" class="nav-link">Checkout</a></li><li><a href="{{ route('order.tracking') }}" class="nav-link">Order Tracking</a></li><li><a href="{{ route('customer.login') }}" class="nav-link">My Account</a></li><li><a href="mailto:{{ $supportEmail }}" class="nav-link">Contact</a></li></ul></div></div>
            <div class="col-lg-3 col-md-6 col-sm-6"><div class="footer-menu"><h5 class="widget-title">Request a Product</h5><p>Can’t find an item? Send us the product name and phone number.</p><form method="POST" action="{{ route('product_requests.store') }}" class="footer-request-form">@csrf<input type="hidden" name="source" value="footer"><div style="position:absolute;left:-10000px;top:auto;width:1px;height:1px;overflow:hidden;" aria-hidden="true"><label>Website</label><input type="text" name="website" value="" tabindex="-1" autocomplete="off"></div><input class="form-control bg-light" name="customer_name" type="text" placeholder="Name" required><input class="form-control bg-light" name="phone" type="text" placeholder="Phone" required><input class="form-control bg-light" name="product_name" type="text" placeholder="Product name" required><button class="btn btn-dark" type="submit">Request</button></form></div></div>
        </div>
    </div>
</footer>
<div id="footer-bottom"><div class="container-fluid"><div class="row"><div class="col-md-6 copyright"><p>{{ $footerCopyright }}</p></div><div class="col-md-6 credit-link text-start text-md-end"><p>Developed by <a href="{{ $footerCreditUrl }}" target="_blank" rel="noopener noreferrer">{{ $footerCreditText }}</a>.</p></div></div></div></div>
...
<script src="{{ asset('foodmart/js/prosan-foodmart.js') }}?v=20260729-1"></script>
</body>
</html>
